...
const { temporaryImageLocation } = require("../constants");
...
class FileManager {
	...
	async processImage(imageName, query) {
		...
		const { Location } = await this.s3.uploadFile(fileNameWithQuery, updatedImageBuffer);
		...
		return { imageUrl: Location, updatedImageBuffer };
	}
