...
/**
 * 
 * Handler for getting proxied and rescaled images.
 * Supports height & width query to get rescaled versions of image.
 * 
 */
class GetImageHandler {
    ...
    /**
     * Handles getting proxied and rescaled images
     * 
    ...
    async handle(req, res) {
        ...
        /**
         * Checks in memory image cache repo if an url to the rescaled image exists.
         */
        ...
        if (imageUrl) {
            ...
            try {
                ...
                /**
                 * Otherwise we rescale the image
                 */
    ...
    async _rescaleImage(req, res) {
        ...
        try {
            /**
             * Gets the image for rescaling and saves it to disc. 
             */
            await this._getImageForRescaling(fileName, operationId);
             ...
             * return 404.
             */
            if (!this._isImage(fileType(readFile))) {
                res.status(404);
                res.end();
            ...
            /**
             * If buffer is image and everything correct, rescale it.
             */
            rescaleFile = sharp(readFile).resize(width, height);
            rescaleFile.pipe(file);
            rescaledImageBuffer = await rescaleFile.toBuffer();
            await rescaleFile.end();

            res.end(rescaledImageBuffer);

            /**
             * Sends rescaled image back to client before saving it to S3 (To speed up request)
            ...
            try {
                /**
                 * Uploads the rescaled image to S3.
                 */
                await uploadRescaledImage(this);
            } catch (err) {
                log.error(err);
            }
         ...
         */
        async function uploadRescaledImage(that, lastResolve) {
            /**
             * Tries to upload rescaled image file to S3 {maxTries} times.
             */
            if (currentTry !== maxTries) {
                log.debug("trying to upload rescaled image, try " + (currentTry + 1));
                currentTry++;

                return new Promise((resolve, reject) => {
                    postRequest = request.post({
                        uri: conf.serviceHttpUrl + constants.endpoints.http.UPLOAD_RESCALED_IMAGE,
                        formData: {
                        ...
                        if (err) {
                             ...
                             */
                            return await uploadRescaledImage(that, lastResolve || resolve);
                        } else {
                            log.debug("Saved rescaled image to s3: ", req.params.imageName);
                            body = JSON.parse(body);

                            /**
                             * Adds uploaded rescaled image url to the in memory image cache repo.
                             */
                ...
                postRequest.end();
                log.debug("was unable to upload rescaled image at this point");
                Promise.reject("");
            }
        }
    }
    ...
    /**
     * Gets image and saves it to disc for image rescaling.
     * 
     * @param {String} imageName 
     * @param {String} operationId 
     */
    _getImageForRescaling(imageName, operationId) {
