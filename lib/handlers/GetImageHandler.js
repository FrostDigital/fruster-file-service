...
class GetImageHandler {
	...
	async handle({ params: { imageName }, query: { height, width, angle } }, res) {
		 ...
		 */
		if (this._noQuery({ height, width, angle }))
		 ...
		 */
		({ height, width, angle } = this._cleanQuery({ height, width, angle }));
	 ...
	 * @param {Object} query
	 */
	_noQuery({ height, width, angle }) {
		if (!height && !width && !angle)
			return true;

		let parsedHeight = Number.parseFloat(height);
		let parsedWidth = Number.parseFloat(width);
		let parsedAngle = Number.parseFloat(angle);

		return (parsedHeight <= 0 && parsedWidth <= 0) || (parsedAngle <= 0 || parsedAngle >= 360);
	}
	 ...
	 * @returns {Object}
	 */
	_cleanQuery({ height, width, angle }) {
