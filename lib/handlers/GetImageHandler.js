...
class GetImageHandler {
	 ...
	 */
	async handle({ params: { imageName }, query: { height, width } }, res) {
		/**
		 ...
		 */
		const imageUrl = this.repo.get(imageName, { height, width });
		...
		if (imageUrl) {
			 ...
			 */
			const fileName = utils.getImageFileNameFromQuery(imageName, { height, width });
			...
			try {
				...
				this.repo.add(imageName, { height, width }, `${conf.imageBaseUri}/${fileName}`);
			} catch (err) {
				/**
				 * Otherwise we resize the image
				 */
				await this._resizeImage(imageName, height, width, res);
			};
		}
	}
	 ...
	 * @param {ExpResponse} res - http response
	 */
	async _resizeImage(imageName, height, width, res) {
		...
		try {
			const { imageUrl, updatedImageBuffer } = await this.fileManager.processImage(imageName, { height, width });
			this.repo.add(imageUrl, { height, width }, `${conf.imageBaseUri}/${imageName}`);
